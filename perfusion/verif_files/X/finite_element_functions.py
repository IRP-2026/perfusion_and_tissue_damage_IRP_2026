...
def allocation_functions_space(mesh_obj, fe_degr, **kwarg):
    ...
    return Vp, Vvel, v_1, v_2, v_3, p, p_1, p_2, p_3
    ...
    if read_inlet_boundary:
        ...
        if model_type == 'acv':
            for i in range(n_labels):
                ...
                # CORRECTION : Localisation correcte des DOFs pour le compartiment 3 (veines)
        ...
        for i in range(n_labels):
            if boundary_labels[i] > 2:
                ...
                if inlet_BC_type == 'DBC':
                    # CORRECTION MAJEURE : On passe V_space (V.sub(0)) au lieu de V
