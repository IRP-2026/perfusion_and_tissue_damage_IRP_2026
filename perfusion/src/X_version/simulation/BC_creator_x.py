...
# Local imports
from ..io import IO_functions, basic_flow_solver_IO
...
def create_BC_creator_parser():
    ...
    parser.add_argument("--config_file", help="path to configuration file (string)",
                        type=str, default='./configs/config_basic_flow_solver_RMCAo.yaml')
    return parser
...
def create_boundary_values_and_map(mesh_file_split):
    ...
    for i in list(boundary_values):
        ...
        matches = np.argwhere(boundary_mapper[:, 1] == int(i))
        
        if matches.size > 0:
            row_index = matches[0][0]
            boundary_map[counter] = int(boundary_mapper[row_index, 0])
        else:
            boundary_map[counter] = 0
...
def populate_boundary_matrix_test(mask, boundary_labels, boundary_map, boundary_values, Q, p, is_clustered):
    ...
    for i in range(n_mask):
        if mask[i] > 0:
            ...
            label = int(boundary_labels[i])
            flow = float(Q[i])
            pressure = float(p[i])

            if is_clustered:
                match_indices = np.argwhere(boundary_values == label)
                mapped_value = int(boundary_map[match_indices[0][0]]) if match_indices.size > 0 else 0
...
def main():
    ...
    # Read mesh
    mesh, subdomains, boundaries = IO_functions.mesh_reader(comm, mesh_file)

    # Volumetric flow rate to the brain [ml / s]
    Q_brain = 10.0

    # Compute surface area for each boundary region
    boundary_labels = list(np.unique(boundaries.values))
